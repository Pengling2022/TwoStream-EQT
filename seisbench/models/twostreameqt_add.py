...
class TwoStreamEQT(WaveformModel):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(self.cnn_blocks)
        print(self.res_cnn_blocks)
        print(self.eqt_mode)
        ...
        self.all_filters = [8, 16, 16, 32, 32, 64, 64]  # Number of filters for the convolutions
        self.all_kernel_sizes = [11, 9, 7, 7, 5, 5, 3]  # Kernel sizes for the convolutions
        self.all_res_cnn_kernels = [3, 3, 3, 3, 2, 3, 2]

        # Determine the maximum number of blocks based on the length of all_filters
        max_blocks = len(self.all_filters)

        # Adjust the cnn_blocks and res_cnn_blocks to not exceed the maximum number of parameters
        effective_cnn_blocks = min(self.cnn_blocks, max_blocks)
        effective_res_cnn_blocks = min(self.res_cnn_blocks, max_blocks)

        # Select filters and res_cnn_kernels based on effective blocks
        self.filters = self.all_filters[:effective_cnn_blocks]
        self.kernel_sizes = self.all_kernel_sizes[:effective_cnn_blocks]
        self.res_cnn_kernels = self.all_res_cnn_kernels[:effective_res_cnn_blocks]
...
class FrequenceDim(nn.Module):
    def __init__(self, in_channels, in_samples , filters, kernel_sizes,res_cnn_kernels,lstm_blocks,
                 drop_rate ,original_compatible, fs=100 , nperseg=80, sigma=0.01):
        super().__init__()
        self.in_channels = in_channels
        ...
        self.gaussian_window = windows.gaussian(self.nperseg, std=self.sigma * self.fs).to('cuda')

        # Dim
        self.dimtrans = DimTrans(in_channels, in_samples)

        # Encoder stack
        self.encoder = Encoder(
            input_channels=int(self.nperseg // 2 + 1)*3,
            filters=self.filters,
            kernel_sizes=self.kernel_sizes,
            in_samples=in_samples,
        )
    ...
    def forward(self,x):
        ...
        # xf_stft = xf_stft.view(batch_size, self.in_channels, xf_stft.shape[1], xf_stft.shape[2])
        # xf_stft = torch.abs(xf_stft)
        # xf_stft = F.interpolate(xf_stft, size=(xf_stft.shape[2], self.in_samples), mode='bilinear')
        # xf_stft = xf_stft.view(batch_size, xf_stft.shape[1] * xf_stft.shape[2], xf_stft.shape[-1])
        ...
        # plt.xlim([0, 6000])
        # plt.subplot(312)
        # plt.imshow(xf[0, :, :].detach().cpu().numpy(), cmap='jet', aspect='auto')
        # plt.subplot(313)
        # plt.imshow(xf_stft[0, :, :].detach().cpu().numpy(), cmap='jet', aspect='auto')
        # plt.show()
...
class TimeDim(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
class DimTrans(nn.Module):
    def __init__(self, in_channels, in_samples, dimtrans_weights=None):
        super(DimTrans, self).__init__()
        self.in_channels = in_channels
        self.in_samples = in_samples
        self.dimtrans_weights = dimtrans_weights

        if self.dimtrans_weights is None:
            # Initialize weights if not provided
            self.dimtrans_weights = nn.Parameter(torch.ones(1, 1, in_samples) / in_samples, requires_grad=True)

    def forward(self, x):
        # Apply weighted sum along the third dimension
        x_weighted = x * self.dimtrans_weights

        # Sum along the third dimension
        x_transformed = torch.sum(x_weighted, dim=2)
