...
def main(weights, targets, batchsize, num_workers, sampling_rate=None):
    ...
    # split = dataset.get_full_dataset()

    # If targets are for instance task, handle noise trace names
    if targets.name == "instance":
        ...
        dataset._metadata["trace_name"].values[
            -len(dataset.datasets[-1]) :
        ] = dataset._metadata["trace_name"][-len(dataset.datasets[-1]) :].apply(
            lambda x: "noise_" + x
        )
        dataset._build_trace_name_to_idx_dict()

    logging.warning("Starting prediction for the entire dataset")
    ...
    for task in ["1"]:
        ...
        # No filtering by split here, since we're predicting on the entire dataset
        ...
        generator = sbg.SteeredGenerator(dataset, task_targets)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "The script will detect which tasks are present based on file names.",
    )
