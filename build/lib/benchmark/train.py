...
def train(config, experiment_name, test_run):
    ...
    # 移除旧的 `gpus` 参数
    trainer_args.pop('gpus', None)

    trainer = pl.Trainer(
        default_root_dir=default_root_dir,
        logger=loggers,
        callbacks=callbacks,
        accelerator="gpu",  # 使用 'gpu' 作为加速器
        devices=trainer_args.get('devices', 1),  # 使用 devices 参数
        **trainer_args,
    )
...
def prepare_data(config, model, test_run):
    ...
    dataset.filter(dataset._metadata["flag"] == True)
...
#     torch.manual_seed(42)
#
#     # Load your config directly without needing command-line arguments
#     config_path = "/mnt/c/Users/33297/Desktop/pick-benchmark-main/configs/stead_crosstwostreameqt.json" # Update this path to your config file
#     with open(config_path, "r") as f:
#         config = json.load(f)
#
#     experiment_name = os.path.basename(config_path)[:-5]
#     test_run = False  # Set this to True for a test run
#
#     train(config, experiment_name, test_run=test_run)
#
#     running_time = str(
#         datetime.timedelta(seconds=time.perf_counter() - code_start_time)
#     )
