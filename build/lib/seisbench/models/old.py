...
class OldTwoStreamEQT(WaveformModel):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fusion_inputsize = {'time':64, 'frequence':64, 'twostream':128}
...
class Fencoder(nn.Module):
    def __init__(self, in_channels, in_samples ,drop_rate ,fs=100 ,nperseg=80):
        super().__init__()
        self.in_channels=in_channels
    ...
    def forward(self,x):
        ...
        # print(xf.shape)
        # plt.imshow(torch.abs(xf[0, 0, :, :]).cpu().numpy(), aspect='auto', origin='lower')
...
#         self.conv1 = nn.Conv1d(in_channels=41, out_channels=16, kernel_size=3, padding=1)
...
#
#     def forward(self, inpf):
#         a_list = torch.unbind(inpf, dim=1)
#
#         # Loop to generate 'a' and concatenate
#         a_concatenated_list = []
#         for a in a_list:
#             a = self.conv1(a)
#             a = nn.ReLU()(a)
...
#             a = nn.ReLU()(a)
#             a = a.squeeze(dim=1)
#             a_concatenated_list.append(a)
#
#         concatenated_tensor = torch.stack(a_concatenated_list, dim=1)
